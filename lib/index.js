...
const app = express();
const port = 3000;
const featuresPath = '././Features/**/*.feature';  // Padrão de busca para arquivos .feature em subpastas
...
function generateNavigation(features, query = '') {
    ...
    let navigation = `<div class="navigation">
        <h1><a href="/">${rootFolderName}</a></h1>
        <form action="/search" method="get">
            <input type="text" name="q" placeholder="Feature name or content..." value="${query}" />
...
function generateSearchResultsHTML(features, allFeatures, query) {
    ...
    let resultsHtml = '<div class="content"><h2>Search Results</h2>';
    ...
    if (features.length) {
        ...
        features.forEach(feature => {
            ...
            processedContent = processedContent.replace(regexGherkin, (match) => `<b>${match}</b><br>`);
...
function cssStyles() {
    return `
        ...
        body {
            ...
            color: #484848; /* Cor de texto inspirada no Airbnb */
        }
        ...
        b {
            color: #007A87; /* Cor para palavras-chave, exemplo com um tom teal */
            font-weight: bold; /* Negrito para destacar */
        }

        /* Realce para termos pesquisados */
        mark {
            background-color: #FF5A5F80; /* Fundo vermelho claro para destacar texto */
        ...
        .content p {
            margin-top: 5px; /* Espaçamento após o título da feature */
            color: #667; /* Cor mais suave para o texto */
        }
    `;
}
